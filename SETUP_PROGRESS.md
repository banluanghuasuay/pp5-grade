...
### ✅ เหลือ → เสร็จครบแล้ว (2026-06-05 · ดูสรุปท้ายไฟล์)
...
## ✅ Multi-tenant deploy ครบ + report room-suffix + update banner (2026-06-05 Sonnet↔Opus)

### A. หัวรายงาน: ห้องเดียวไม่แสดง "/1"
classLabel ทุกหน้าใช้ `ชั้น${name_th}/${room_number}` ไม่มีเงื่อนไข → ป.1 ห้องเดียวโชว์ "ป.1/1"
แก้: helper `reportRoomSuffix(classroomId)` ใน `lib/current-term.ts` — นับห้องในชั้น+ปี
(`classrooms_staff_read` = is_staff → ครูก็นับครบ) คืน "/N" เมื่อ 2+ ห้อง · คืน "" เมื่อห้องเดียว
ใช้ 9 จุด classLabel (8 report pages: pp5 · pp5-class · pp6 · grade-summary · score-table ·
student-eval · attendance ×2 · attendance-by-subject) + pp5 selector dropdown (multiRoom guard)
- หน้ากรอกข้อมูล (setup/*) ทำถูกอยู่แล้ว — `ClassroomSelector` ซ่อน dropdown ห้องเมื่อ rooms.length<=1
- `8f76eb8`

### B. แถบแจ้งเตือนเวอร์ชันใหม่ (in-app update notification)
โรงเรียน deploy จาก **fork ของตัวเอง** → ไม่รู้ว่ามีอัปเดต · ทำระบบเทียบเวอร์ชัน:
- `apps/admin/version.json` = แหล่งเวอร์ชันเดียว (ผู้ดูแลแก้เลข = แจ้งอัปเดต)
- `lib/version-check.ts` — fetch upstream version.json (raw.githubusercontent /main · cache 1h ·
  fail-silent) เทียบ semver กับเวอร์ชัน bake ตอน build · `getVersionStatus()` + `checkForUpdate()`
- `_components/update-banner.tsx`(+client) — แถบเขียวทุกหน้า admin · ปิดได้ (localStorage keyed by
  version) · เฉพาะ isAdmin · `<Suspense>` ไม่ block paint
- `_components/version-status.tsx` — สถานะท้าย dashboard (ล่าสุด/มีอัปเดต/offline) แสดงตลอด
- `docs/RELEASING.md` — วิธี bump version.json + ข้อจำกัด (fork เก่าต้อง Sync ครั้งแรกก่อนถึงเห็นแถบ)
- หลักการ: fork ที่ยังไม่ sync มี version.json เก่า bake ไว้ → fetch upstream เทียบ → เห็นช่องว่าง
- `9a70eee`

### C. ไฟล์รายวิชาสำเร็จรูป + DEPLOY.md ครบ
- `docs/import-subjects-2569.sql` — seed รายวิชา+แผนการเรียน ป.1-ม.3 (core+activity) รันไฟล์เดียวจบ
  สร้างจาก export โรงเรียนต้นแบบ · cleaned: `ค13103/ค13104`→`ก` · ลบ sem-2 ค้าง (4 core + 2 เพิ่มเติม)
- DEPLOY.md เพิ่ม: GitHub signup+Fork (4.1-4.2) · Storage bucket `school-logos` public (2.2) ·
  RLS popup "Run without RLS" · Project URL ไม่มี `/rest/v1/` · การอัปเดต (Sync fork) · นำเข้าวิชา 2 วิธี
- setup.sql เพิ่ม `INSERT INTO schools` placeholder ('โรงเรียน...')
- cleanup DB ต้นทาง: rename `ค13103/ค13104`→`ก` + DELETE sem-2 (admin รันแล้ว 2026-06-05)
- `e052d27` import+DEPLOY · `34e6066` setup.sql school · `b827061` storage · `aa28be6` GitHub/Fork

### ⚡ Performance (จาก session ก่อน · ยืนยัน)
Vercel region → **sin1** (Singapore) ให้ตรงกับ Supabase ap-southeast-1 · เดิม iad1/US = ~300ms/query
ต้อง Redeploy เอง · user ยืนยัน "เร็วขึ้นเยอะ"

### 🧰 ชุดเครื่องมือ deploy โรงเรียนใหม่ (ครบ)
`setup.sql` · `create-first-admin.sql` · `import-subjects-2569.sql` (วิชาสำเร็จรูป) ·
`export-subjects.sql` (วิชาจาก DB เดิม) · `docs/DEPLOY.md` (คู่มือเต็ม) · `docs/RELEASING.md` (ออกเวอร์ชัน)

### หมายเหตุ pattern (สำคัญ)
- เทียบเวอร์ชัน fork: version.json bake ตอน build + fetch upstream main เทียบ — ไม่ต้องมี backend/DB
