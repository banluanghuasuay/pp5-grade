...
export function ResetPasswordForm({ teacherId }: { teacherId: string }) {
  const [state, formAction] = useActionState(resetTeacherPassword, initialState);

  return (
    <form
      action={formAction}
      className="space-y-4"
      // Remount form after success so the password field clears
    ...
    >
      ...
      <Field
        ...
        hint="แสดงเป็น plain text เพื่อ admin บอกครูตอนพบกัน"
      >
        <Input
          id="reset_password"
          name="password"
